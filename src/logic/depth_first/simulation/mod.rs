...
impl DSimulation {
    ...
    pub fn capture_duration(self, duration: Duration) -> Self {
        Self {
    ...
    pub fn run(self) -> ArrayVec<DDirection, 4> {
        ...
        if self.capture {
            let capture_time = if let Some(duration) = self.capture_max_duration {
        ...
        let simulation_time = if let Some(duration) = self.simulation_max_duration {
            DTreeTime::new(duration)
        } else {
            DTreeTime::default()
        };
...
mod tests {
    ...
    fn test_basic_simulation() {
        ...
        let directions = DSimulation::new(state)
            .capture(false)
            .simulation_duration(Duration::from_millis(250))
            .run();
